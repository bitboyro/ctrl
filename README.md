...
```yaml
ctrl:
  version: "0.3.0"        # pinned ctrl version; `ctrl check` warns on mismatch
...
```bash
./install.sh v0.3.0     # specific version
...
```
warn  ctrl version (0.3.0) differs from ctrl.version declared in ctrl.yaml (0.2.5)
```
